...
class productController extends Controller
{
    public function index()
    {
     $user_id = Auth::user()->id;
      $products = Product::with('users')->where('user_id',$user_id)->get();
    ...
    public function insert(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
          ...
          return redirect()->route('vendor.product.index')->with('flash_message_success','product has added successfully');
    }
      ...
      if ($request->isMethod('post')) {
        ...
        $product->brand_id = $data['brand_id'];
         ...
         if ($request->hasFile('main_image')) {
          ...
          $product->save();
          return redirect()->route('vendor.product.index')->with('flash_message_success','product has update successfully');
        }
    ...
    public function productImage($id)
    {
      ...
      return view('vendor.product.product_image',compact('productData'));
    }
    ...
    public function productImageUpload(Request $request,$id)
    {
      $this->validate($request, [
        ...
        $form->save();
        return redirect()->route('vendor.product.index')->with('flash_message_success','product  Image has added successfully');;
    }
    ...
    public function productAttribute(Request $request,$id)
    {
        ...
        foreach($data['sku'] as $key => $val){
          if (!empty($val)) {
                ...
                return back()->with('flash_message_error',' SKU already exists ! Please add another SKU');
                  ...
                  // if ($attrCountSize>0) {
                  //    return redirect('/vendor/product/add-attributetes/'.$id)->with('flash_message_error',' Size already exists ! Please add another Size');
                  // }
      ...
      return view('vendor.product.product_attribute',compact('productData','productAttributes'));
    }
